...
def run():
    ...
    records=dict(old_records); seeded=updated=0; recoverable_multi_pool=0
    ...
    for state_key,meta in tokens.items():
        ...
        current=None; verified=False; current_pair=None
        if isinstance(currow,dict):
            current_pair=currow.get("pair_address")
            if _norm_pair(current_pair)==_norm_pair(entry_pair):
                current=currow.get("price_usd"); verified=current not in (None,0,0.0)
        if entry_pair and not verified and len(live_token_pairs.get(canonical_key,set()))>1:
        ...
        records[target_key]={**rec,"chain":chain,"token":token,"first_seen":meta.get("first_seen") or rec.get("first_seen"),
            ...
            "current_pair_address":current_pair if verified else rec.get("current_pair_address"),"current_price_usd":current,
            ...
            "checkpoints":checkpoints,"history":history,"updated_at":now_s if verified else rec.get("updated_at"),
    ...
    payload={"version":6,"method":"IMMUTABLE_PERFORMANCE_SINCE_DISCOVERY_EXACT_PAIR","updated_at":now_s,"tracked_tokens":len(records),"seeded_from_discovery_state":seeded,"updated_this_run":updated,"verified_positive_now":positive,"verified_negative_now":negative,"verified_flat_now":flat,"exact_pair_index_mode":"CHAIN_TOKEN_PAIR","multi_pool_tokens_seen":sum(1 for v in live_token_pairs.values() if len(v)>1),"anti_erasure_rule":"NON_EMPTY_TRACK_RECORD_MAY_NEVER_BE_REPLACED_BY_EMPTY_OUTPUT","tokens":records}
    ...
    leaderboard={"updated_at":now_s,"verified_count":len(leaders),"verified_winners":positive,"verified_losers":negative,"verified_flat":flat,"win_rate_pct":win_rate,"best_current_return_pct":leaders[0].get("current_return_pct") if leaders else None,"best_peak_return_pct":max((float(x.get("peak_return_pct") or 0) for x in leaders),default=None),"all_discoveries_hypothetical_investment_usd":all_investment,"verified_cohort_investment_usd":verified_investment,"verified_cohort_current_value_usd":verified_value,"verified_cohort_profit_usd":verified_profit,"verified_cohort_roi_pct":verified_roi,"unverified_or_not_currently_measurable_count":unverified,"portfolio_rule":"$1 AT IMMUTABLE DISCOVERY ENTRY; CURRENT P/L ONLY COUNTED WHEN EXACT PAIR IS CURRENTLY VERIFIED","rows":leaders}
    _write(DATA/"performance-leaderboard.json",leaderboard)
    _write(DATA/"performance-measurement-report.json",{"updated_at":now_s,"tracked_tokens":len(records),"seeded":seeded,"updated":updated,"verified_positive_now":positive,"verified_negative_now":negative,"verified_flat_now":flat,"verified_exact_pair_now":len(verified),"win_rate_pct":win_rate,"legacy_missing_pair":sum(1 for x in records.values() if x.get("pair_identity_status")!="LOCKED"),"exact_pair_index_mode":"CHAIN_TOKEN_PAIR","multi_pool_tokens_seen":sum(1 for v in live_token_pairs.values() if len(v)>1),"all_discoveries_hypothetical_investment_usd":all_investment,"verified_cohort_investment_usd":verified_investment,"verified_cohort_current_value_usd":verified_value,"verified_cohort_profit_usd":verified_profit,"verified_cohort_roi_pct":verified_roi,"unverified_or_not_currently_measurable_count":unverified})
    print(json.dumps({"tracked_tokens":len(records),"updated_this_run":updated,"verified_exact_pair_now":len(verified),"multi_pool_tokens_seen":sum(1 for v in live_token_pairs.values() if len(v)>1),"win_rate_pct":win_rate},indent=2))
    return payload
