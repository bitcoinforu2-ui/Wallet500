"""Immutable rejected/held-candidate learning ledger.

Every candidate that is blocked, failed, quarantined, or held at a material gate
remains observable for later outcome learning. First decision evidence is immutable;
later exact-pair observations append.
"""
...
DATA=Path('data'); LEDGER=DATA/'rejected-candidate-ledger.json'; SUMMARY=DATA/'rejected-candidate-learning-summary.json'
STATIC_SOURCES={
...
def _key(row:dict[str,Any])->str:
 chain,token,pair=_identity(row);return f'{chain}|{token}|{pair}'
...
def _decision_rows()->list[tuple[str,dict[str,Any]]]:
 ...
 for row in _rows(_load(DATA/'fresh-solana-survival.json',[])):
  ...
  elif gate=='PENDING':out.append(('LIVE_SURVIVAL_PENDING',row))
 return out

def update()->dict[str,Any]:
 now=datetime.now(timezone.utc).isoformat();old=_load(LEDGER,{});records=old.get('records') if isinstance(old,dict) and isinstance(old.get('records'),dict) else {};created=0;observations_added=0;current_seen=set();source_counts={}
 for source,row in _decision_rows():
  key=_key(row)
  if key.startswith('||'):continue
  current_seen.add(key);source_counts[source]=source_counts.get(source,0)+1;snap=_snapshot(row,source,now);rec=records.get(key)
  if not isinstance(rec,dict):
   records[key]={'identity':{'chain':snap['chain'],'token':snap['token'],'pair_address':snap['pair_address']},'first_rejected_at':now,'first_reject_source':source,'first_decision_class':snap['decision_class'],'first_reject_snapshot':snap,'observations':[snap],'latest_observation':snap,'classification':'UNRESOLVED_REJECT'};created+=1;observations_added+=1;continue
  obs=rec.get('observations') if isinstance(rec.get('observations'),list) else [];fingerprint=(snap.get('observed_at'),source,snap.get('price_usd'),snap.get('liquidity_usd'));existing={(x.get('observed_at'),x.get('source'),x.get('price_usd'),x.get('liquidity_usd')) for x in obs if isinstance(x,dict)}
  if fingerprint not in existing:obs.append(snap);observations_added+=1
  rec['observations']=obs[-500:];rec['latest_observation']=snap;records[key]=rec
 payload={'schema_version':2,'policy':'IMMUTABLE_FIRST_REJECT_APPEND_ONLY_OBSERVATIONS_EXACT_PAIR_LOCKED','updated_at':now,'records_count':len(records),'created_this_run':created,'observations_added_this_run':observations_added,'records':records};LEDGER.write_text(json.dumps(payload,indent=2));summary={'updated_at':now,'records':len(records),'created_this_run':created,'observations_added_this_run':observations_added,'currently_rejected_or_quarantined':len(current_seen),'source_counts':source_counts,'unresolved':sum(1 for r in records.values() if str(r.get('classification') or '').startswith('UNRESOLVED'))};SUMMARY.write_text(json.dumps(summary,indent=2));print(json.dumps(summary,indent=2));return payload
