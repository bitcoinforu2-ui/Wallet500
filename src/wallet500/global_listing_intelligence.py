...
UA = {"User-Agent": "Wallet500-GlobalListing/1.0", "Accept": "application/json,text/html,*/*"}

# Public official surfaces. HTML sources are evidence feeds only; if a site
# changes or blocks automation it fails visibly and contributes zero candidates.
OFFICIAL_PAGES = [
...
# Conservative base58 address range; market-data validation downstream decides
# whether it is a real Solana mint. Do not auto-promote symbol-only observations.
...
def _infer_chain(token: str) -> str | None:
    ...
    if EVM_RE.fullmatch(token):
        # An address alone cannot distinguish Ethereum from BSC. Feed both into
        # research/deep scan; downstream market discovery will retain only the
        # chain where a valid exact pair exists.
        return "evm_unknown"
    if SOL_RE.fullmatch(token):
        return "solana"
    return None
...
def _extract_addresses(text: str):
    ...
    # Solana regex on full HTML can catch unrelated base58 blobs. Keep only
    # values near token/mint/contract/address wording to reduce false positives.
    lower = (text or "").lower()
    for m in SOL_RE.finditer(text or ""):
        a, b = max(0, m.start()-100), min(len(text), m.end()+100)
        ctx = lower[a:b]
        if not any(w in ctx for w in ("mint", "contract", "token", "address")):
            continue
        token = m.group(0)
        ...
        try:
            ...
            for row in rows:
                ...
                if not token:
                    continue
                observations.append({
                    "observed_at": ts, "source":"moonshot", "surface":f"MOONSHOT_{view.upper()}",
                    "stage":view.upper(), "token":token, "chain":_infer_chain(token),
                    "raw":row, "source_url":url,
                })
    ...
    for surface, exchange, url in OFFICIAL_PAGES:
        try:
            ...
            for token in addresses:
                observations.append({
                    "observed_at":ts, "source":exchange, "surface":surface,
                    "stage":"PRE_LISTING_OR_NEW_LISTING_PUBLIC_SURFACE", "token":token,
                    "chain":_infer_chain(token), "source_url":url,
                })
        except Exception as e:
...
def _key(obs: dict) -> str:
    raw = f"{obs.get('source')}|{obs.get('surface')}|{obs.get('token')}"
...
def _deep_scan_rows(observations):
    ...
    for o in observations:
        ...
        if not token or not chain:
            continue
        chains = ("ethereum", "bsc") if chain == "evm_unknown" else (chain,)
        for c in chains:
            k = f"{c}:{token.lower() if c in {'ethereum','bsc'} else token}"
            if k in seen:
                continue
            seen.add(k)
            rows.append({
                "chain":c,
                "token":token,
                "source":"GLOBAL_LISTING_INTELLIGENCE",
                "listing_source":o.get("source"),
                "listing_surface":o.get("surface"),
                "listing_first_observed_at":o.get("observed_at"),
...
def _merge_watchlist(auto_rows):
    ...
    if not isinstance(current, list):
        current = []
    # Preserve all user/manual entries and replace only prior auto-listing rows.
    manual = [x for x in current if isinstance(x, dict) and x.get("source") != "GLOBAL_LISTING_INTELLIGENCE"]
    merged = manual + auto_rows
    # Bound auto discovery footprint so a malformed external page cannot flood
    # the scanner. Existing immutable listing ledger remains complete.
...
def run():
    ...
    for o in observations:
        ...
        rec = records.get(k)
        if rec is None:
            rec = {"first_seen_at":ts,"first_observation":o,"history":[]}
            records[k] = rec; new_count += 1
        rec["last_seen_at"] = ts
        rec["last_observation"] = o
        hist = rec.get("history") if isinstance(rec.get("history"), list) else []
        hist.append({"at":ts,"stage":o.get("stage"),"source":o.get("source"),"surface":o.get("surface")})
        rec["history"] = hist[-200:]
    ledger.update({
        "version":1,"updated_at":ts,"truth_label":"PUBLIC_LISTING_DISCOVERY_EVIDENCE_ONLY",
        "records":records,
        "policy":"Listing/roadmap/launchpad presence never bypasses Wallet500 production filters or constitutes a buy signal.",
    })
    _write(LEDGER, ledger)

    deep_rows = _deep_scan_rows(observations)
    manual_count, auto_count = _merge_watchlist(deep_rows)
    health = ah + bh
    radar = {
        "version":1,"updated_at":ts,"mode":"GLOBAL_EXCHANGE_PRELISTING_DISCOVERY_FORWARD_ONLY",
        "sources_requested":len(health),"sources_healthy":sum(1 for x in health if x.get("ok")),
        "observations":len(observations),"new_immutable_records":new_count,
        "deep_scan_candidates":auto_count,"manual_watchlist_preserved":manual_count,
        "source_health":health,
        "deep_scan_policy":[
            "contract/mint identity required",
            "symbol-only listing observations remain research only",
            "EVM unknown-chain addresses are tested on Ethereum and BSC",
            "existing Wallet500 liquidity >= $50K, exact-pair, survival, risk, holder/cluster and execution gates remain mandatory",
            "no listing source may promote directly to Qualified/Live/Paper Truth",
        ],
    }
    _write(RADAR, radar)
