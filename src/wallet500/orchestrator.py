...
def run():
    ...
    core=run_core();now=datetime.now(timezone.utc).isoformat();cex=run_cex_revival(out,now);dna=run_catalyst_dna(out,now);tm=run_time_machine(out,now)
