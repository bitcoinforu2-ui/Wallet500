...
def run():
    ...
    # `updated_at` is freshness evidence for the completed output, so stamp it
    # after all long-running stages finish. Keeping the run-start timestamp here
    # would make a healthy recovery older than the watchdog freshness window by
    # construction whenever the scan itself takes >5 minutes.
