...
def run():
    ...
    for e in entries:
        ...
        px=mark['price_usd']
        try:val=float(e['quantity'])*px; cost=float(e.get('cost_usd') or 0)
        except:val=float('nan'); cost=0.0
        if not math.isfinite(val) or val<0 or not math.isfinite(cost) or cost<=0:
