...
def _update_discovery_state(out: Path, universe: list[dict], snapshots: list[dict], next_pages: dict, now: str):
    ...
    for row in universe:
        ...
        if prev:
            ...
            entry_price=prev.get("entry_price_usd")
            tracking_started_at=prev.get("tracking_started_at")
            ...
            tracking_started_at=now if entry_price is not None else None
            legacy=False
...
def _update_outcomes(out: Path, state: dict, snapshots: list[dict], now: str) -> dict:
    ...
    for key,s in snap.items():
        meta=(state.get("tokens") or {}).get(key) or {}
        entry=meta.get("entry_price_usd")
        current=s.get("price_usd")
        if entry in (None,0,0.0) or current in (None,0,0.0):
            continue
        ...
        peak=max(float(rec.get("peak_price_usd") or current),float(current))
        low=min(float(rec.get("low_price_usd") or current),float(current))
        checkpoints=rec.get("checkpoints") if isinstance(rec.get("checkpoints"),dict) else {}
        for mins,label in horizons:
            if age_min>=mins and label not in checkpoints:
                checkpoints[label]={"price_usd":current,"return_pct":_pct(current,entry),"captured_at":now}
        history=rec.get("history") if isinstance(rec.get("history"),list) else []
        history.append({"observed_at":now,"price_usd":current,"return_pct":_pct(current,entry),"liquidity_usd":s.get("liquidity_usd"),"volume_h1":s.get("volume_h1")})
        history=history[-200:]
        records[key]={"chain":meta.get("chain") or s.get("chain"),"token":meta.get("token") or s.get("token") or s.get("mint"),"first_seen":meta.get("first_seen"),"tracking_started_at":tracking_started_at,"legacy_price_tracking":bool(meta.get("legacy_price_tracking",False)),"entry_price_usd":entry,"current_price_usd":current,"current_return_pct":_pct(current,entry),"peak_price_usd":peak,"peak_return_pct":_pct(peak,entry),"low_price_usd":low,"low_return_pct":_pct(low,entry),"age_minutes":round(age_min,2),"checkpoints":checkpoints,"history":history,"updated_at":now}
        updated+=1
    ...
    _write(path,tracker)
    _write(out/"signal-outcomes.json",list(records.values()))
    return tracker
...
def run():
    ...
    outcomes=_update_outcomes(out,state,snapshots,now)
    ...
    result={"mode":"market-first","verified_only":True,"chains":market["chains"],"counts":market["counts"],"universe":len(universe),"snapshots":len(snapshots),"anomalies":len(anomalies),"qualified":len(qualified),"rejected":len(rejected),"watchlist":len(watch),"historical_review_queued":len(review),"outcome_tracked":outcomes.get("tracked_tokens",0),"manual_research_cases":len(manual_watch),"discovery":state.get("last_run",{}),"unique_tokens_total":state.get("unique_tokens_total",0),"scan_runs_total":state.get("runs",0),"updated_at":now}
