...
def _market_status(live):
    reasons=[]
    if not live:return 'UNAVAILABLE_NOW',['CURRENT_EXACT_PAIR_QUERY_UNAVAILABLE']
...
def run():
    ...
    failed=_confirmed_failed();cohort=[];quote_eligible=[];unknown=[];now=datetime.now(timezone.utc).isoformat();dead=tradable=blocked=0
    for key,r in records.items():
        ...
        if not chain or not token or not pair or entry<=0:
            unknown.append({'key':key,'reason':'MISSING_IMMUTABLE_ENTRY_OR_PAIR'});continue
        live=snapshot(chain,token,pair);status,reasons=_market_status(live);live_price=(live or {}).get('price_usd');ret=_ret(live_price,entry)
        confirmed_dead=_k(chain,token,pair) in failed or (live is not None and float(live.get('price_usd') or 0)<=0)
        if confirmed_dead:
            ret=-100.0;marked=0.0;dead+=1;status='CONFIRMED_DEAD_CURRENT_VALUE_ZERO';reasons=list(dict.fromkeys(reasons+['CONFIRMED_TERMINAL_PAIR_EVIDENCE']))
        elif ret is not None:
            marked=max(0.0,POSITION_USD*(1+ret/100))
            if status=='CURRENTLY_TRADABLE':tradable+=1
            else:blocked+=1
        else:
            # No current quote is not proof of death. Preserve the token and disclose it as unknown;
            # never turn a transient API miss into a fabricated -100%.
    ...
    all_n=len(records)
    payload={'updated_at':now,'method':'PERFORMANCE_SINCE_DISCOVERY_V7_IMMUTABLE_LEDGER_SURVIVORSHIP_CORRECTED','position_size_usd':POSITION_USD,'all_discoveries_count':all_n,'immutable_entry_pair_records':all_n-len([u for u in unknown if u['reason']=='MISSING_IMMUTABLE_ENTRY_OR_PAIR']),'tracked_cohort_count':n,'unknown_not_scored_count':len(unknown),'dead_zero_value_count':dead,'currently_tradable_count':tradable,'currently_blocked_but_marked_count':blocked,'market_execution_plausible_count':len(quote_eligible),'not_realizable_now_count':dead+blocked,'survivorship_bias_policy':'SOURCE IS IMMUTABLE OUTCOME LEDGER, NOT CURRENT SURVIVOR LEADERBOARD. CONFIRMED DEAD PAIRS REMAIN AT $0/-100%. TRANSIENT UNAVAILABLE QUOTES ARE UNKNOWN, NEVER FABRICATED AS DEATH.','current_return_portfolio':{'count':n,'investment_usd':round(invested,6),'marked_value_usd':round(value,6),'pnl_usd':round(pnl,6),'roi_pct':round(roi,4),'status':'PERFORMANCE_SINCE_DISCOVERY_CURRENT_RETURN_NOT_EXECUTED_PNL'},'current_realizability_reference':{'count':n,'investment_usd':round(invested,6),'marked_value_usd':round(value,6),'pnl_usd':round(pnl,6),'roi_pct':round(roi,4),'status':'IMMUTABLE_LEDGER_SURVIVORSHIP_CORRECTED_CURRENT_RETURN'},'historical_backtest_verified':False,'historical_backtest_status':'NOT_BACKTEST_VERIFIED','tracked_discoveries':all_n,'eligible_now':n,'blocked_now':dead+blocked,'invested_usd':0.0,'current_value_usd':0.0,'roi_pct':0.0,'truth_note':'Every immutable record with valid entry price + locked pair is considered. Confirmed terminal pairs count as $0/-100%; surviving exact pairs use current price; unresolved current marks are disclosed UNKNOWN rather than silently dropped or called dead.','important_limit':'Research hold-to-now reference, not proof of executable historical buys/sells.','unknown_rows':unknown,'rows':sorted(cohort,key=lambda x:x['current_return_pct'],reverse=True),'plausible_rows':sorted(quote_eligible,key=lambda x:x['current_return_pct'],reverse=True)}
