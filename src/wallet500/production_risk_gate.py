...
from .accuracy_contracts import candidate_authority_states, wallet_shadow_evidence
...
from .social_precursor import assess_social_precursors
...
def _current_execution_liquidity(candidate):
    """Liquidity used for the production gate.

    Concentrated/DLMM/CLMM pools must provide verified execution depth; provider
    TVL/reserve is never accepted as executable liquidity for those pool types.
    Non-concentrated exact-pair rows retain the legacy reserve gate until a depth
    provider is available, and are explicitly labeled as such in the result.
    """
    value, _, _ = safe_production_liquidity(candidate)
    return value
...
def _history_metrics(candidate, outcomes):
    ...
    ls=[]
    for x in history:
        value, _, eligible = safe_production_liquidity(x if isinstance(x,dict) else {})
        if eligible and value>0: ls.append(value)
...
def evaluate(candidate,outcomes):
    ...
    authority={'mint':'NOT_APPLICABLE','freeze':'NOT_APPLICABLE'}
    if str(candidate.get('chain') or candidate.get('network') or '').lower() == 'solana':
        authority=candidate_authority_states(candidate)
        if authority['mint']=='UNKNOWN': critical.append('MINT_AUTHORITY_TRUTH_UNKNOWN_FAIL_CLOSED')
        elif authority['mint']=='ACTIVE': critical.append('MINT_AUTHORITY_ACTIVE_HARD_BLOCK')
        if authority['freeze']=='UNKNOWN': reasons.append('FREEZE_AUTHORITY_TRUTH_UNKNOWN_NOT_SAFE')
        elif authority['freeze']=='ACTIVE': reasons.append('FREEZE_AUTHORITY_ACTIVE_SECURITY_RISK')
    sig=_pre_rug_signature(candidate,current,previous); conc=_concentration(candidate); wallet_shadow=wallet_shadow_evidence(candidate)
    social_shadow=assess_social_precursors(candidate,candidate.get('social_evidence') or [],now=candidate.get('social_as_of'))
    ...
    return {**candidate,'production_risk_gate':'BLOCKED' if blocked else ('CAUTION' if reasons else 'PASS'),'production_risk_blocked':blocked,'production_risk_critical':list(dict.fromkeys(critical)),'production_risk_reasons':list(dict.fromkeys(critical+reasons)),'production_live_liquidity_usd':current,'production_execution_pool_liquidity_usd':current,'production_liquidity_gate_metric':'VERIFIED_EXECUTION_DEPTH_USD_5PCT_OR_NON_CONCENTRATED_LEGACY_GATE','production_liquidity_truth_source':liquidity_source,'production_liquidity_gate_eligible':liquidity_eligible,'production_pool_tvl_usd':candidate.get('pool_tvl_usd') or candidate.get('provider_reported_pool_value_usd'),'production_dex_total_liquidity_usd':candidate.get('dex_total_liquidity_usd'),'production_previous_liquidity_usd':previous or None,'production_peak_observed_liquidity_usd':peak or None,'production_liquidity_retention_from_peak':round(retention_peak,6) if retention_peak is not None else None,'production_history_points':m['history_points'],'pre_rug_danger_score':sig['danger_score'],'pre_rug_exit_warning':sig['exit_warning'],'pre_rug_turnover_h1':round(sig['turnover_h1'],6),'pre_rug_sell_buy_ratio_h1':round(sig['sell_buy_ratio_h1'],6),'pre_rug_signals':sig['signals'],'lp_removal_protection_verified':sig['lp_verified'],'liquidity_drain_holder_cluster_verified':conc['verified'],'liquidity_drain_insider_linked_supply_pct':conc['insider_pct'],'liquidity_drain_top10_supply_pct':conc['top10_pct'],'liquidity_drain_signals':conc['signals']+conc['critical'],'solana_mint_authority_state':authority['mint'],'solana_freeze_authority_state':authority['freeze'],'wallet_discovery_shadow':wallet_shadow,'social_precursor_shadow':social_shadow}
...
def apply(output_dir='data'):
    ...
    if isinstance(summary,dict):
        summary['production_risk_gate']={'min_execution_liquidity_usd':int(MIN_TRADABLE_LIQUIDITY_USD),'min_live_liquidity_usd':int(MIN_TRADABLE_LIQUIDITY_USD),'liquidity_gate_metric':'VERIFIED_EXECUTION_DEPTH_USD_5PCT_OR_NON_CONCENTRATED_LEGACY_GATE','pool_tvl_never_equals_execution_depth':True,'concentrated_pool_unverified_depth_policy':'FAIL_CLOSED_HARD_BLOCK','dex_total_liquidity_is_informational_only':True,'active_before_gate':len(active),'active_after_gate':len(passed),'blocked_now':len(blocked),'exit_warnings_now':sum(bool(x.get('pre_rug_exit_warning')) for x in evaluations),'hard_rule':'CONCENTRATED_POOL_REQUIRES_VERIFIED_EXECUTION_DEPTH; NON_CONCENTRATED LEGACY EXACT-PAIR RESERVE REMAINS TEMPORARY','solana_mintability_rule':'MINT_AUTHORITY_MUST_BE_REVOKED_NULL; UNKNOWN_FAILS_CLOSED; FREEZE_UNKNOWN_NEVER_REPORTED_SAFE','evacuation_rule':'LOSS_OF_EXECUTABLE_LIQUIDITY_OVERRIDES PRICE VOLUME BUYS AND ANOMALY SCORE','lp_rule':'LP LOCK ONLY PROTECTS AGAINST LP REMOVAL; IT DOES NOT PROTECT QUOTE-SIDE LIQUIDITY FROM INSIDER/HOLDER DUMP DRAIN','drain_rule':'HOLDER/CLUSTER CONCENTRATION IS A SEPARATE REQUIRED DRAIN-RISK CONTROL; >=35% LINKED INSIDER OR >=70% TOP10 IS HARD BLOCK WHEN VERIFIED DATA IS PRESENT.','wallet_discovery_rule':'WALLET_CLUSTER_EVIDENCE_IS_RANKING_SHADOW_ONLY; NEVER_AUTO_PROMOTE','social_precursor_rule':'SOCIAL/INFLUENCER/CATALYST_EVIDENCE_IS_RESEARCH_SHADOW_ONLY; EXACT_CHAIN_CONTRACT_REQUIRED; NEVER_AUTO_PROMOTE_OR_CREATE_REAL_ALERT'}; summary['active_qualified']=len(passed); summary['watchlist']=len(filtered); _write(out/'run-summary.json',summary)
