...
BLOCKED_BASE_TOKENS={
    "solana":{"So11111111111111111111111111111111111111112","EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGKZwyTDt1v","Es9vMFrzaCERmJfrF4H2FYD9iG6vGvL5JZtJm6Gq5tQ"},
...
def _get(url:str,timeout:int=20,retries:int=3,headers=None):
    last=None
    for attempt in range(max(1,retries)):
        try:
            h={"Accept":"application/json","User-Agent":"Wallet500/1.5"}
            ...
            if e.code==429 and attempt+1<retries:
                ...
                try:delay=max(2.5,float(retry)) if retry else 3.5*(attempt+1)
...
def _dex_latest(wanted,limit,rows,seen,counts,filtered,errors):
    endpoints=("/token-profiles/latest/v1","/token-boosts/latest/v1","/token-boosts/top/v1","/community-takeovers/latest/v1")
    for ep in endpoints:
...
def _birdeye_new_listings(wanted,limit,rows,seen,counts,filtered,errors):
    if not BIRDEYE_KEY:
        errors.append({"source":"birdeye:new_listing","status":"UNCONFIGURED","error":"BIRDEYE_API_KEY_MISSING"})
        return
    for chain in sorted(wanted):
        ...
        url=f"{BIRDEYE}/defi/v2/tokens/new_listing?{urlencode(params)}"
        try:data=_get(url,headers={"X-API-KEY":BIRDEYE_KEY,"x-chain":bchain})
...
def _source_stats(rows):
    ...
    for row in rows:
        ...
        c=int(row.get("source_confirmations") or len(srcs) or 1)
...
def discover_tokens(chains=CHAINS,limit_per_chain:int=120,start_pages=None,pages_per_run:int=3,max_page:int=DEEP_MAX_PAGE)->tuple[list[dict],dict]:
    ...
    # Discovery V2: independent external screeners first, then broad pool crawls.
