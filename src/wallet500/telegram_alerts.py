...
def _merge_display_context(active: dict, real: dict) -> dict:
    merged = dict(active)
    for key in (
        "symbol", "name", "token_address", "dex", "dex_url", "price_usd",
        "liquidity_usd", "execution_pool_liquidity_usd", "market_age_days", "score",
        "source_lanes", "source_lane_count", "evidence_envelope_status", "evidence_ready",
        "evidence_positive_lanes", "evidence_verified_lanes", "status",
        "actionable_research_alert", "first_alert_at",
    ):
        value = real.get(key)
...
def _message(row: dict, tier: str, sent_at: str | None = None, alert_event_id: str | None = None) -> str:
    ...
    lines.extend([
        ...
        "Pair identity: EXACT LOCK ✅", f"Pair checked: {pair_checked}", f"Market age: ≥{market_age}d ✅",
        ...
        "Holder/Cluster evidence: COMPLETE PASS", "Actionable research alert: YES ✅",
    ])
    if source_lanes:
    ...
    report = {
        ...
        "policy": {
            ...
            "requires": [
                ...
                "pair_identity_locked=true", "pair_address=locked_pair_address", "holder_cluster_production_status=PASS",
                "holder_cluster_verification_complete=true", f"verified_execution_liquidity>={int(MIN_LIQUIDITY_USD)}",
                f"volume_h1>={int(MIN_VOLUME_H1_USD)}", f"activity_h1>={MIN_ACTIVITY_H1}", "risk not HIGH/CRITICAL",
            ],
