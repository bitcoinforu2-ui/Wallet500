...
def _message(row: dict, tier: str) -> str:
    ...
    liquidity = row.get("live_liquidity_usd") or row.get("liquidity_usd")
...
def run() -> dict:
    ...
    candidates_path = out / "active-qualified-candidates.json"
    ...
    for row in candidates:
        ...
        if not tier:
            continue
        key = _token_key(row)
        fingerprint = f"{tier}:{row.get('qualified_at') or row.get('observed_at') or ''}"
        eligible.append({"key": key, "tier": tier})
        if sent.get(key, {}).get("fingerprint") == fingerprint:
            continue
        if not configured:
            continue
        try:
            _send(bot_token, chat_id, _message(row, tier))
            sent[key] = {"fingerprint": fingerprint, "tier": tier, "sent_at": now}
            delivered.append({"key": key, "tier": tier})
    ...
    report = {
        ...
        "policy": {
            ...
            "dedupe": "one alert per qualification fingerprint",
        },
    }
