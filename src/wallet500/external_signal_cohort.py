...
def collect_gecko(errors):
    ...
    for net,chain in [('solana','solana'),('eth','ethereum'),('bsc','bsc')]:
        try:
            ...
            errors.append({'source':'GECKOTERMINAL_TRENDING','chain':chain,'failure_code':'EXTERNAL_SOURCE_FETCH_FAILED','error_type':type(e).__name__})
    return out

def collect_dexscreener(errors):
    out=[]
    for endpoint,src in [('token-boosts/top/v1','DEXSCREENER_BOOSTED'),('token-profiles/latest/v1','DEXSCREENER_PROFILES')]:
        try:
            ...
            errors.append({'source':src,'failure_code':'EXTERNAL_SOURCE_FETCH_FAILED','error_type':type(e).__name__})
    return out

def run():
    t=now(); state=load(OUT,{})
    if not state:
        state={'method':'EXTERNAL_SIGNAL_COHORT_6H_V2_TRUTH_WINDOW','started_at':t.isoformat(),'ends_at':(t+timedelta(hours=6)).isoformat(),'production_change':False,'records':{}}
    records=state.setdefault('records',{})
    end=datetime.fromisoformat(state['ends_at'].replace('Z','+00:00'))
    errors=[]; fresh=[]; status='ACTIVE'
    if t<=end:
        fresh=collect_gecko(errors)+collect_dexscreener(errors)
        for x in fresh:
        ...
        state['last_ingest_at']=t.isoformat();state['last_ingest_count']=len(fresh)
    else:
        status='COMPLETE'
    by={}
    for r in records.values():by[r['source']]=by.get(r['source'],0)+1
    state['source_counts']=by;state['status']=status;state['last_source_errors']=errors;state['source_error_count']=len(errors)
    OUT.write_text(json.dumps(state,indent=2,ensure_ascii=False)+'\n')
    elapsed=max(0.0,(t-datetime.fromisoformat(state['started_at'].replace('Z','+00:00'))).total_seconds()/60)
    summary={'updated_at':t.isoformat(),'method':state['method'],'status':status,'started_at':state['started_at'],'ends_at':state['ends_at'],'elapsed_minutes':round(elapsed,2),'total_unique_source_candidates':len(records),'source_counts':by,'last_ingest_count':state.get('last_ingest_count',0),'source_error_count':len(errors),'source_errors':errors,'production_change':False,'truth_window_enforced':True,'rule':'External sources nominate candidates only; Wallet500 hard gates remain authoritative; no observations are added after ends_at.'}
