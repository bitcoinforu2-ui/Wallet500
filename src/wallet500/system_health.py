...
def _diagnose(name, check, now):
    ...
    if name == 'primary_scan':
        ...
        diag.update(failure_code='WALLET_FORENSICS_CAPABILITY_GAP',severity='HIGH',blocks_production=False,expected={'when_active':'at least one verified wallet candidate or explicit supported-chain scan result'},actual={'active_expected':check.get('active_candidates_expected'),'active_seen':check.get('active_candidates_seen'),'verified_wallet_candidates':check.get('verified_wallet_candidates'),'solana_candidates_scanned':check.get('solana_candidates_scanned'),'evm_candidates_deferred':check.get('evm_candidates_deferred')},recommended_action='IMPLEMENT_OR_RESTORE_EVM_WALLET_FORENSICS_AND_VERIFY_EVIDENCE_YIELD')
    else:
...
def build_health(output_dir='data', now=None):
    ...
    source=wallet.get('source') if isinstance(wallet,dict) else None; seen=int(wallet.get('active_candidates_seen',0) or 0) if isinstance(wallet,dict) else 0; verified=int(wallet.get('verified_wallet_candidates',0) or 0) if isinstance(wallet,dict) else 0; sol_scanned=int(wallet.get('solana_candidates_scanned',0) or 0) if isinstance(wallet,dict) else 0; evm_deferred=int(wallet.get('evm_candidates_deferred',0) or 0) if isinstance(wallet,dict) else 0
    ...
    checks['wallet_forensics']={'status':'HEALTHY' if wallet_ok else 'DEGRADED','age_seconds':round(wallet_age,1) if wallet_age is not None else None,'source':source,'active_candidates_expected':active,'active_candidates_seen':seen,'verified_wallet_candidates':verified,'solana_candidates_scanned':sol_scanned,'evm_candidates_deferred':evm_deferred}
    capability_ok=(active==0) or (seen>0 and (verified>0 or sol_scanned>0) and evm_deferred<seen)
    checks['wallet_forensics_capability']={'status':'HEALTHY' if capability_ok else 'DEGRADED','active_candidates_expected':active,'active_candidates_seen':seen,'verified_wallet_candidates':verified,'solana_candidates_scanned':sol_scanned,'evm_candidates_deferred':evm_deferred,'interpretation':'PIPELINE_RUNNING_IS_NOT_THE_SAME_AS_EVIDENCE_CAPABILITY_WORKING'}
    checks={name:_diagnose(name,check,now) for name,check in checks.items()}
    pipeline_names={'primary_scan','publish_pipeline','old_coin_revival','new_token_lab','holder_cluster_fail_closed','liquidity_policy','holder_cluster_evidence_coverage','wallet_forensics'}
    capability_names={'wallet_forensics_capability'}
    def _status(names):
    ...
    pipeline_health=_status(pipeline_names); capability_health=_status(capability_names)
    ...
    system_blockers=sum(1 for x in failures if x.get('blocks_production')); candidate_gate_blocks=hblock+hqua
    failure_summary={'count':len(failures),'production_blockers':system_blockers,'system_production_blockers':system_blockers,'candidate_gate_blocks':candidate_gate_blocks,'codes':[x.get('failure_code') for x in failures]}
    gate_summary={'holder_cluster_input':hinput,'verified_promoted':hprom,'quarantined':hqua,'blocked':hblock,'candidate_gate_blocks':candidate_gate_blocks,'production_authorized_candidates':hprom,'interpretation':'Candidate gate blocks are correct fail-closed decisions and are separate from system health failures.'}
    ...
    return {'version':5,'diagnostic_contract':'FAIL_LOUD_FAIL_SPECIFIC_FAIL_TRACEABLE','updated_at':now.isoformat(),'overall':overall,'pipeline_health':pipeline_health,'capability_health':capability_health,'failure_summary':failure_summary,'gate_summary':gate_summary,'failures':failures,'checks':checks,'lane_metrics':lane_metrics}
...
def run(output_dir='data'):
    ...
    if isinstance(summary,dict):
        summary['system_health']={'overall':health['overall'],'pipeline_health':health['pipeline_health'],'capability_health':health['capability_health'],'failure_summary':health['failure_summary'],'gate_summary':health['gate_summary'],'failures':health['failures'],'checks':health['checks'],'lane_metrics':health['lane_metrics']}; _write(out/'run-summary.json',summary)
