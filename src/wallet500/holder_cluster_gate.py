...
def _sol_holders(token):
 ...
 for row,info in zip(token_accounts,values):
  ...
  item={**row,'owner':owner}; resolved.append(item)
...
def run():
 src=_load(DATA/INPUT_FILE,{}); rows=src.get('rows',src if isinstance(src,list) else []); rows=rows if isinstance(rows,list) else []
 out=[]; seen=set()
 for r in rows:
 ...
 now=datetime.now(timezone.utc).isoformat(); payload={'updated_at':now,'method':'WALLET500_HOLDER_CLUSTER_PRETRADE_GATE_V1_1_OWNER_RESOLVED','input_file':INPUT_FILE,'truth_note':'Solana concentration is aggregated by resolved owner wallet, not raw SPL token account. No cluster PASS without independently verified transfer/funding/deployer evidence. EVM holder data is not fabricated.','rows':out}
