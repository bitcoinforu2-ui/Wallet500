...
def _holder_role(chain, address, pair_address=None, registry=None):
    ...
    if registered:
        registered["excluded_from_whale_concentration"] = bool(
            registered["role"] in EXCLUDABLE_ROLES and registered["confidence"] >= ROLE_EXCLUSION_MIN_CONFIDENCE
        )
        return registered
    if addr in INFRA_EXCLUSIONS:
        return {
            "role": "KNOWN_INFRASTRUCTURE",
            "label": "Environment infrastructure exclusion",
            "confidence": 1.0,
            "source": "HOLDER_CLUSTER_INFRA_EXCLUSIONS",
            "excluded_from_whale_concentration": True,
        }
...
def _role_aware_distribution(chain, holders, pair_address=None, registry=None):
    ...
    for rank, holder in enumerate(holders, 1):
        ...
        row.update({
            "gross_rank": rank,
            "holder_role": role["role"],
            "holder_role_label": role.get("label"),
            "holder_role_confidence": round(float(role.get("confidence") or 0), 4),
            "holder_role_source": role.get("source"),
            "excluded_from_whale_concentration": bool(role.get("excluded_from_whale_concentration")),
        })
    ...
    return {
        ...
        "gross_top1_pct": sum(gross[:1]),
        "gross_top5_pct": sum(gross[:5]),
        "gross_top10_pct": sum(gross[:10]),
        "adjusted_top1_pct": sum(real_pcts[:1]),
        "adjusted_top5_pct": sum(real_pcts[:5]),
        "adjusted_top10_pct": sum(real_pcts[:10]),
        "adjusted_top10_complete": len(real_holders) >= 10,
        "known_infrastructure_pct": sum(float(x.get("pct") or 0) for x in excluded),
        "dex_lp_pct": role_pct("DEX_LP_POOL"),
        "cex_custody_pct": role_pct("CEX_CUSTODY"),
        "burn_lock_pct": role_pct("BURN_LOCK"),
        "excluded_holder_count": len(excluded),
        "unknown_holder_pct_observed": sum(float(x.get("pct") or 0) for x in classified if x.get("holder_role") == "UNKNOWN"),
    }
...
def _evm_logs_resilient(urls, token, a, b):
    ...
    meta = {
        "queries": 1 + lm["queries"] + rm["queries"],
        "splits": 1 + lm["splits"] + rm["splits"],
        "failed_range": lm.get("failed_range") or rm.get("failed_range"),
        "rpc_endpoints_used": max(lm["rpc_endpoints_used"], rm["rpc_endpoints_used"]),
    }
...
def _sol_holders(token):
    ...
    return owners, resolved, {
        "supply": total,
        "largest_accounts_returned": len(token_accounts),
        "owners_resolved": rc,
        "owner_resolution_complete": rc == len(token_accounts) and rc > 0,
        "reason": "SOLANA_OWNER_RESOLUTION_COMPLETE" if rc == len(token_accounts) and rc > 0 else "SOLANA_OWNER_RESOLUTION_INCOMPLETE",
    }
...
def _components(holders, graph, exclusions):
    ...
    for e in graph:
        a = str(e.get("from") or "").lower()
        b = str(e.get("to") or "").lower()
        n = int(e.get("transfer_count") or 0)
        if not a or not b or a == b or a not in nodes or b not in nodes:
            continue
        adj[a].add(b)
        adj[b].add(a)
        edge_counts[tuple(sorted((a, b)))] += n
    seen = set()
    out = []
...
def _evm_holders(chain, token, row):
    ...
    start, start_source, start_verified = _evm_start_block(row, latest_i)
    balances = defaultdict(int)
    edges = defaultdict(int)
    logs_seen = chunks = queries = splits = 0
    used = {latest_url} if latest_url else set()
    for a in range(start, latest_i + 1, EVM_CHUNK):
        b = min(latest_i, a + EVM_CHUNK - 1)
        chunks += 1
        logs, qmeta = _evm_logs_resilient(urls, token, a, b)
        ...
        for log in logs:
            ...
            f = _addr(topics[1])
            t = _addr(topics[2])
            value = _as_int_hex(log.get("data") or "0x0")
            logs_seen += 1
            if f and f != ZERO:
                balances[f] -= value
            if t and t != ZERO:
                balances[t] += value
            if f and t and f != ZERO and t != ZERO:
                edges[(f, t)] += 1
    total_supply, supply_url = _evm_total_supply(urls, token)
    if supply_url:
        used.add(supply_url)
    positive = [(o, a) for o, a in balances.items() if a > 0]
    ...
    clusters = _components(holders, graph, exclusions)
    complete = bool(start_verified and total_supply > 0)
...
def analyze(row):
    ...
    if c in ("SOL", "SOLANA"):
        ...
        if not meta.get("complete"):
            reasons.append(meta.get("reason", "EVM_EVIDENCE_INCOMPLETE"))
    else:
        holders = []
        reasons.append("UNSUPPORTED_CHAIN")

    distribution = _role_aware_distribution(c, holders, pair, _role_registry())
    ...
    if c in ("ETH", "ETHEREUM", "BSC", "BNB"):
        clusters = _components(holders, graph, role_exclusions | INFRA_EXCLUSIONS | {ZERO, str(token or "").lower(), str(pair or "").lower()})
        clusters = corroborate_clusters(clusters, graph, deployer_evidence, funding)

    top1 = distribution["adjusted_top1_pct"]
    top5 = distribution["adjusted_top5_pct"]
    top10 = distribution["adjusted_top10_pct"]
    if not holders:
    ...
    verification_complete = bool((sol_complete or evm_complete) and holders)
    if c in ("SOL", "SOLANA") and not sol_complete:
        reasons.append("SOME_TOKEN_ACCOUNT_OWNERS_UNRESOLVED")
    if verification_complete and not distribution["adjusted_top10_complete"]:
        reasons.append("ADJUSTED_TOP10_REPLACEMENT_INCOMPLETE_REVIEW_ONLY")
    if top1 > MAX_TOP1:
        reasons.append("TOP1_OWNER_CONCENTRATION_HIGH" if verification_complete else "TOP1_CONCENTRATION_HIGH_REVIEW_ONLY")
    if top5 > MAX_TOP5:
        reasons.append("TOP5_OWNER_CONCENTRATION_HIGH" if verification_complete else "TOP5_CONCENTRATION_HIGH_REVIEW_ONLY")
    if top10 > MAX_TOP10:
        reasons.append("TOP10_OWNER_CONCENTRATION_HIGH" if verification_complete else "TOP10_CONCENTRATION_HIGH_REVIEW_ONLY")
    if distribution["cex_custody_pct"] >= CEX_CUSTODY_MONITOR_PCT:
        reasons.append("CEX_CUSTODY_SUPPLY_GE_30PCT_MONITOR_FLOW")
    ...
    blockable = corroborated if evm_complete else []
    if linked:
        reasons.append("LINKED_TOP_HOLDER_COMPONENT_REQUIRES_CORROBORATION")
    if corroborated:
        reasons.append("CORROBORATED_LINKED_HOLDER_CLUSTER_GE_20PCT" if evm_complete else "CORROBORATED_LINKED_HOLDER_CLUSTER_REVIEW_ONLY_INCOMPLETE_LEDGER")

    hard_block = (
        verification_complete
        and any(x in reasons for x in ("TOP1_OWNER_CONCENTRATION_HIGH", "TOP5_OWNER_CONCENTRATION_HIGH", "TOP10_OWNER_CONCENTRATION_HIGH"))
    ) or bool(blockable)
    review_only_reason = any(x.endswith("REVIEW_ONLY") for x in reasons)
    needs_review = bool(linked) or not verification_complete or review_only_reason
    ...
    return {
        ...
        "gross_top1_pct": round(distribution["gross_top1_pct"], 4),
        "gross_top5_pct": round(distribution["gross_top5_pct"], 4),
        "gross_top10_pct": round(distribution["gross_top10_pct"], 4),
        "adjusted_real_top1_pct": round(top1, 4),
        "adjusted_real_top5_pct": round(top5, 4),
        "adjusted_real_top10_pct": round(top10, 4),
        "adjusted_top10_complete": distribution["adjusted_top10_complete"],
        "known_infrastructure_pct": round(distribution["known_infrastructure_pct"], 4),
        "dex_lp_pct": round(distribution["dex_lp_pct"], 4),
        "cex_custody_pct": round(distribution["cex_custody_pct"], 4),
        "burn_lock_pct": round(distribution["burn_lock_pct"], 4),
        "unknown_holder_pct_observed": round(distribution["unknown_holder_pct_observed"], 4),
        "holder_role_policy": "ONLY_EXACT_HIGH_CONFIDENCE_INFRASTRUCTURE_ROLES_ARE_EXCLUDED;UNKNOWN_FAILS_CLOSED;CEX_CUSTODY_MONITORED_SEPARATELY",
        "cluster_verified": bool(corroborated),
        "linked_cluster_candidates": linked,
        "corroborated_cluster_risks": corroborated,
        "blockable_cluster_risks": blockable,
        "deployer_evidence": deployer_evidence,
        "verified_native_funding_edges": funding,
...
def run():
    src = _load(DATA / INPUT_FILE, {})
    rows = _rows_from_source(src)
    out = []
    seen = set()
    for r in rows:
        if not isinstance(r, dict):
            continue
        chain = r.get("chain")
        token = r.get("token") or r.get("token_address") or r.get("mint")
        pair = r.get("pair_address") or r.get("locked_pair_address")
        key = (str(chain), str(token), str(pair))
        if not chain or not token or key in seen:
            continue
        seen.add(key)
    ...
    payload = {
        "updated_at": now,
        "method": "WALLET500_HOLDER_CLUSTER_PRETRADE_GATE_V2_ROLE_AWARE",
        "input_file": INPUT_FILE,
        "truth_note": "Gross concentration is diagnostic only. Hard concentration gates use adjusted non-infrastructure holders after exact high-confidence role classification. Unknown addresses remain counted. CEX custody is excluded from whale concentration but monitored as a separate flow/concentration risk.",
        "role_exclusion_min_confidence": ROLE_EXCLUSION_MIN_CONFIDENCE,
        "rows": out,
    }
    _write(DATA / "holder-cluster-gate.json", payload)
    _write(DATA / "holder-cluster-gate-summary.json", {
        "updated_at": now,
        "input_file": INPUT_FILE,
        "checked": len(out),
        "block": sum(x["status"] == "BLOCK" for x in out),
        "review": sum(x["status"] == "REVIEW" for x in out),
        "pass": sum(x["status"] == "PASS" for x in out),
        "verification_complete": sum(x["verification_complete"] for x in out),
        "method": payload["method"],
    })
    print(json.dumps({"checked": len(out), "pass": sum(x["status"] == "PASS" for x in out), "review": sum(x["status"] == "REVIEW" for x in out), "block": sum(x["status"] == "BLOCK" for x in out)}, indent=2))
    return payload
