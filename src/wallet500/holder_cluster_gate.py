...
DATA=Path('data');TOP_N=20;MAX_TOP1=20.0;MAX_TOP5=50.0;MAX_TOP10=65.0
CLUSTER_REVIEW_PCT=float(os.getenv('HOLDER_CLUSTER_REVIEW_PCT','10'));CLUSTER_BLOCK_PCT=float(os.getenv('HOLDER_CLUSTER_BLOCK_PCT','20'))
TRANSFER_TOPIC='0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef';TOTAL_SUPPLY_SELECTOR='0x18160ddd';ZERO='0x0000000000000000000000000000000000000000'
...
def _urls(*values):
 ...
 return out
RPC={'SOLANA':_urls(os.getenv('SOLANA_RPC_URL'),*DEFAULT_RPC['SOLANA']),'ETHEREUM':_urls(os.getenv('ETHEREUM_RPC_URL'),os.getenv('ETH_RPC_URL'),os.getenv('ETH_RPC_FALLBACK_URLS'),*DEFAULT_RPC['ETHEREUM']),'BSC':_urls(os.getenv('BSC_RPC_URL'),os.getenv('BNB_RPC_URL'),os.getenv('BSC_RPC_FALLBACK_URLS'),*DEFAULT_RPC['BSC'])};RPC['SOL']=RPC['SOLANA'];RPC['ETH']=RPC['ETHEREUM'];RPC['BNB']=RPC['BSC']
INPUT_FILE=os.getenv('HOLDER_CLUSTER_INPUT','active-qualified-candidates.json');EVM_LOOKBACK=int(os.getenv('HOLDER_EVM_LOOKBACK_BLOCKS','50000'));EVM_CHUNK=max(100,int(os.getenv('HOLDER_EVM_LOG_CHUNK','2000')));EVM_MIN_LOG_SPAN=max(10,int(os.getenv('HOLDER_EVM_MIN_LOG_SPAN','100')));INFRA_EXCLUSIONS={x.strip().lower() for x in os.getenv('HOLDER_CLUSTER_INFRA_EXCLUSIONS','').split(',') if x.strip()}
...
def _rpc_url(url,method,params):
 ...
 try:
  ...
  with urlopen(req,timeout=25) as r:x=json.loads(r.read().decode());return None if x.get('error') else x.get('result')
...
def _evm_logs_resilient(urls,token,a,b):
 ...
 if logs is not None:return logs,{'queries':1,'splits':0,'failed_range':None,'rpc_endpoints_used':1 if url else 0}
 if b-a+1<=EVM_MIN_LOG_SPAN:return None,{'queries':1,'splits':0,'failed_range':[a,b],'rpc_endpoints_used':0}
 mid=(a+b)//2;left,lm=_evm_logs_resilient(urls,token,a,mid);right,rm=_evm_logs_resilient(urls,token,mid+1,b)
...
def _sol_holders(token):
 supply=_rpc('getTokenSupply',[token,{'commitment':'confirmed'}]) or {};total=float((supply.get('value') or {}).get('uiAmount') or 0);largest=_rpc('getTokenLargestAccounts',[token,{'commitment':'confirmed'}])
 if not isinstance(largest,dict):return [],[],{'supply':total,'largest_accounts_returned':0,'owners_resolved':0,'owner_resolution_complete':False,'reason':'SOLANA_LARGEST_ACCOUNTS_RPC_UNAVAILABLE'}
 token_accounts=[]
 ...
 if not token_accounts:return [],[],{'supply':total,'largest_accounts_returned':0,'owners_resolved':0,'owner_resolution_complete':False,'reason':'SOLANA_LARGEST_ACCOUNTS_EMPTY'}
...
def _components(holders,graph,exclusions):
 ...
 for root in sorted(nodes):
  ...
  cs=set(comp);transfers=sum(n for (a,b),n in edge_counts.items() if a in cs and b in cs);out.append({'wallets':sorted(comp),'wallet_count':len(comp),'combined_pct':round(sum(pct.get(x,0) for x in comp),4),'direct_transfer_count':transfers,'evidence':'DIRECT_TOKEN_TRANSFERS_AMONG_CURRENT_TOP_HOLDERS','ownership_claim':False})
...
def _evm_holders(chain,token,row):
 ...
 latest_i=int(latest,16);start,start_source,start_verified=_evm_start_block(row,latest_i);balances=defaultdict(int);edges=defaultdict(int);logs_seen=0;chunks=0;queries=0;splits=0;used={latest_url} if latest_url else set()
 for a in range(start,latest_i+1,EVM_CHUNK):
  b=min(latest_i,a+EVM_CHUNK-1);chunks+=1;logs,meta=_evm_logs_resilient(urls,token,a,b);queries+=meta['queries'];splits+=meta['splits']
  if logs is None:return [],[],[],{'complete':False,'reason':'EVM_LOG_RANGE_UNAVAILABLE','from_block':start,'to_block':latest_i,'failed_chunk':meta.get('failed_range') or [a,b],'chunks_completed':chunks-1,'adaptive_log_queries':queries,'adaptive_splits':splits,'start_block_verified':start_verified,'start_block_source':start_source,'rpc_candidates':len(urls),'rpc_endpoints_used':len(used)}
  for log in logs:
...
def analyze(row):
 ...
 hard_block=(verification_complete and any(x in reasons for x in ('TOP1_OWNER_CONCENTRATION_HIGH','TOP5_OWNER_CONCENTRATION_HIGH','TOP10_OWNER_CONCENTRATION_HIGH'))) or bool(blockable);needs_review=bool(linked) or not verification_complete or any(x.endswith('REVIEW_ONLY') for x in reasons);status='BLOCK' if hard_block else ('REVIEW' if needs_review else 'PASS');level='ONCHAIN_OWNER_CONCENTRATION_RESOLVED' if sol_complete and holders else ('FULL_EVM_TRANSFER_LEDGER' if evm_complete and holders else ('BOUNDED_ONCHAIN_TRANSFER_LEDGER' if holders else 'INSUFFICIENT_EVIDENCE'))
...
def run():
 ...
 now=datetime.now(timezone.utc).isoformat();payload={'updated_at':now,'method':'WALLET500_HOLDER_CLUSTER_PRETRADE_GATE_V1_7_ADAPTIVE_RPC','input_file':INPUT_FILE,'truth_note':'PASS still requires complete holder evidence. Adaptive RPC range splitting improves evidence availability only; an unresolved smallest range remains REVIEW and thresholds are unchanged.','rows':out};_write(DATA/'holder-cluster-gate.json',payload);_write(DATA/'holder-cluster-gate-summary.json',{'updated_at':now,'input_file':INPUT_FILE,'checked':len(out),'block':sum(x['status']=='BLOCK' for x in out),'review':sum(x['status']=='REVIEW' for x in out),'pass':sum(x['status']=='PASS' for x in out),'verification_complete':sum(x['verification_complete'] for x in out)});print(json.dumps({'checked':len(out),'pass':sum(x['status']=='PASS' for x in out),'review':sum(x['status']=='REVIEW' for x in out),'block':sum(x['status']=='BLOCK' for x in out)},indent=2));return payload
