...
def _evm_total_supply(url,token):
 x=_rpc_url(url,'eth_call',[{'to':token,'data':TOTAL_SUPPLY_SELECTOR},'latest'])
 return _as_int_hex(x) if x else 0
...
def _components(holders,graph,exclusions):
 ...
 nodes=set(pct)-set(exclusions); adj=defaultdict(set); edge_counts=defaultdict(int)
 for e in graph:
...
def _evm_holders(chain,token,row):
 ...
 total_supply=_evm_total_supply(url,token)
 positive=[(o,a) for o,a in balances.items() if a>0]
 denom=total_supply if total_supply>0 else sum(a for _,a in positive)
