...
RPC={'SOLANA':os.getenv('SOLANA_RPC_URL','https://api.mainnet-beta.solana.com'),'SOL':os.getenv('SOLANA_RPC_URL','https://api.mainnet-beta.solana.com'),'ETHEREUM':os.getenv('ETH_RPC_URL',''),'ETH':os.getenv('ETH_RPC_URL',''),'BSC':os.getenv('BSC_RPC_URL',''),'BNB':os.getenv('BSC_RPC_URL','')}
...
def _components(holders,graph,exclusions):
 ...
 for root in sorted(nodes):
  ...
  comp_set=set(comp);transfers=sum(n for (a,b),n in edge_counts.items() if a in comp_set and b in comp_set)
...
def _evm_holders(chain,token,row):
 ...
 total_supply=_evm_total_supply(url,token);positive=[(o,a) for o,a in balances.items() if a>0];denom=total_supply if total_supply>0 else sum(a for _,a in positive)
 holders=[{'owner':o,'raw_amount':str(a),'pct':a/denom*100 if denom else 0} for o,a in positive];holders.sort(key=lambda x:int(x['raw_amount']),reverse=True);holders=holders[:TOP_N]
 graph=[{'from':f,'to':t,'transfer_count':n} for (f,t),n in sorted(edges.items(),key=lambda x:x[1],reverse=True)[:500]]
 exclusions=set(INFRA_EXCLUSIONS)|{ZERO,token.lower(),str(row.get('pair_address') or row.get('locked_pair_address') or '').lower()};clusters=_components(holders,graph,exclusions);complete=bool(start_verified and total_supply>0)
...
def analyze(row):
 ...
 sol_complete=c in ('SOL','SOLANA') and bool(meta.get('owner_resolution_complete'));evm_complete=c in ('ETH','ETHEREUM','BSC','BNB') and bool(meta.get('complete'))
 ...
 hard_block=(hard_concentration and any(x in reasons for x in ('TOP1_OWNER_CONCENTRATION_HIGH','TOP5_OWNER_CONCENTRATION_HIGH','TOP10_OWNER_CONCENTRATION_HIGH'))) or bool(blockable);status='BLOCK' if hard_block else 'REVIEW'
...
def run():
 ...
 now=datetime.now(timezone.utc).isoformat();payload={'updated_at':now,'method':'WALLET500_HOLDER_CLUSTER_PRETRADE_GATE_V1_4_CORROBORATION','input_file':INPUT_FILE,'truth_note':'Direct token-transfer components are linkage evidence only. Corroboration requires a verified deployer distribution or independently verified common native funding. Even corroborated EVM linkage cannot hard-block unless the holder ledger itself is complete from a verified start block with verified total supply. Common ownership is never claimed.','rows':out}
