...
def discover_solana_candidate_wallets(adapter, candidate: dict, signatures_limit: int = 10) -> dict:
    ...
    base = {
        ...
        "method": "VERIFIED_POOL_TRANSACTION_SIGNERS",
        "signatures_requested": signatures_limit,
        "signatures_seen": 0,
    ...
    for row in signatures:
        ...
        if not sig:
            continue
        try:
            tx = adapter.transaction(sig)
        except Exception:
            continue
        if not tx:
            continue
        loaded += 1
        block_time = tx.get("blockTime") or row.get("blockTime")
        for wallet in extract_signers(tx):
            if wallet == pair:
                continue
            counts[wallet] += 1
            tx_refs[wallet].append(sig)
    ...
    for wallet, appearances in counts.most_common(25):
        wallets.append({
            "address": wallet,
            "signer_appearances": appearances,
            "first_block_time": first_seen.get(wallet),
            "last_block_time": last_seen.get(wallet),
            "sample_signatures": tx_refs[wallet][:5],
            "candidate_reason": "SIGNED_RECENT_ACTIVE_POOL_TRANSACTION",
            "verified": True,
        })
...
def run(output_dir: str = "data", max_tokens: int = 3, signatures_limit: int = 10) -> dict:
    out = Path(output_dir)
    ...
    for candidate in solana:
        rows.append(discover_solana_candidate_wallets(adapter, candidate, signatures_limit))

    for candidate in active:
        if candidate.get("chain") in {"ethereum", "bsc"}:
            skipped.append({
                "chain": candidate.get("chain"),
                "token": candidate.get("token") or candidate.get("mint"),
                "status": "ADAPTER_NOT_YET_AVAILABLE",
                "reason": "No verified EVM transaction adapter is deployed yet; no wallet data fabricated.",
            })

    wallet_count = sum(len(x.get("wallets") or []) for x in rows)
    status = {
        "version": 1,
        "updated_at": now,
        "method": "VERIFIED_POOL_TRANSACTION_SIGNERS",
        "active_candidates_seen": len(active),
        "solana_candidates_scanned": len(rows),
        "verified_wallet_candidates": wallet_count,
        "evm_candidates_deferred": len(skipped),
        "solana": rows,
        "deferred": skipped,
    }
