...
def test_first_successful_fallback_is_baseline_only(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(xf, "fetch_syndication", lambda: (rows, {"status": "OK_SYNDICATION", "count": 1}))
    ...
    assert state["x_syndication_bootstrapped"] is True
...
def test_new_fallback_post_flows_to_priority_queue(monkeypatch, tmp_path):
    ...
    _write(xf.STATE_PATH, {
        "version": 1,
        "bootstrapped": True,
        "x_syndication_bootstrapped": True,
        "seen_posts": ["x:99"],
        "tokens": {},
    })
    _write(xf.CALLS_PATH, {"events": []})
