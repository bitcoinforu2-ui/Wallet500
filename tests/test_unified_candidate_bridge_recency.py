...
def test_gate_ticker_never_suppresses_different_exact_asset(tmp_path, monkeypatch):
    paths = {
        "SPOT": tmp_path / "spot.json",
        "CEX_SPOT_IDENTITY": tmp_path / "cex_identity.json",
        "ALPHA": tmp_path / "alpha.json",
    ...
    paths["CEX_SPOT_IDENTITY"].write_text(json.dumps({
        "candidates": [{
            ...
            "execution_pool_liquidity_usd": 90000,
            "dex_pool_count": 1,
            "dex_liquidity_pools_top5": [{
                "chain": "ethereum",
                "token_address": "0xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
            ...
            "markets": [{"volume_24h": 100000, "volume_comparable_usd_like": True}],
        }]
    }), encoding="utf-8")
    paths["ALPHA"].write_text('{"candidates":[]}', encoding="utf-8")
    paths["BUY_REGISTRY"].write_text('{"entries":{}}', encoding="utf-8")
    ...
    bridge.main()
    result = json.loads(paths["OUT"].read_text(encoding="utf-8"))
    assets = {(x.get("network"), x.get("contract")) for x in result["candidates"]}
    assert ("bsc", "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa") in assets
    assert ("ethereum", "0xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb") in assets
