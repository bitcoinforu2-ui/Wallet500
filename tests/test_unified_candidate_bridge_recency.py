...
def _empty_bridge_paths(tmp_path):
    paths = {
        "SPOT": tmp_path / "spot.json",
        "CEX_SPOT_IDENTITY": tmp_path / "cex_identity.json",
        "ALPHA": tmp_path / "alpha.json",
...
def test_fresh_genesis_prebreakout_enters_unified_dynamic_lane(tmp_path, monkeypatch):
    ...
    paths["GENESIS"].write_text(
        json.dumps({
            ...
            "candidates": [{
                "chain": "solana",
                "token": "GenesisMint111111111111111111111111111111",
                "pair_address": "GenesisPair111111111111111111111111111111",
                "symbol": "GEN",
                "url": "https://dexscreener.com/solana/GenesisPair",
                "price_usd": 0.001,
                "liquidity_usd": 85000,
                "pair_created_at": "2026-09-22T10:00:00+00:00",
                "status": "WATCH",
                "age_band": "PRIME_GENESIS_WINDOW",
                "extension_band": "NORMAL",
                "safety": {"hard_blocks": [], "research_only_reasons": ["UNKNOWN_LP_INTEGRITY"]},
                "genesis_score": 69,
                "shadow_score": 81,
                "prebreakout": {
                    "score": 78,
                    "coverage_pct": 63,
                    "priority_ready": True,
                    "signals": ["PRESSURE_BEFORE_PRICE_EXTENSION", "LIQUIDITY_COMMITMENT"],
                    "risks": [],
                },
            }],
        }),
    ...
    result = json.loads(paths["OUT"].read_text(encoding="utf-8"))
    assert result["counts"]["genesis_prebreakout"] == 1
    assert result["counts"]["genesis_source_fresh"] is True
    row = next(x for x in result["candidates"] if x.get("candidate_type") == "GENESIS_PREBREAKOUT")
    assert row["genesis_final_buy_lane"] is True
    assert row["telegram_policy"] == "FINAL_BUY_ONLY"
    assert row["deep_investigation"] is True
    assert row["prebreakout_score"] == 78

    events = json.loads(paths["EVENTS"].read_text(encoding="utf-8"))
    assert any(
        e.get("canonical_event_id", "").startswith("genesis-prebreakout:")
        for e in events["events"]
    )


def test_stale_genesis_prebreakout_fails_closed(tmp_path, monkeypatch):
    paths = _empty_bridge_paths(tmp_path)
    paths["GENESIS"].write_text(
        json.dumps({
            ...
            "candidates": [{
                "chain": "solana",
                "token": "StaleGenesisMint111111111111111111111111111",
                "pair_address": "StaleGenesisPair11111111111111111111111111",
                "symbol": "OLDG",
    ...
    class FixedDatetime(datetime):
        @classmethod
        def now(cls, tz=None):
            return cls(2026, 9, 22, 12, 0, tzinfo=timezone.utc)

    monkeypatch.setattr(bridge, "datetime", FixedDatetime)
    bridge.main()

    result = json.loads(paths["OUT"].read_text(encoding="utf-8"))
    assert result["counts"]["genesis_prebreakout"] == 0
    assert result["counts"]["genesis_source_fresh"] is False
    assert result["counts"]["genesis_stale_excluded"] == 1


def test_bundle_dominated_genesis_never_bridges():
    row = {
        "chain": "solana",
        "token": "BundleMint111111111111111111111111111111",
        "pair_address": "BundlePair111111111111111111111111111111",
        "liquidity_usd": 90000,
        "status": "WATCH",
        "age_band": "PRIME_GENESIS_WINDOW",
        "extension_band": "NORMAL",
