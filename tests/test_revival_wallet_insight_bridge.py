...
def test_bridge_rotates_previous_cycle_without_hindsight(monkeypatch):
    rows = [
        {"identity": f"solana|T{i}|P{i}", "symbol": f"T{i}", "candidate_status": "EVIDENCE_READY", "classification": "DATA_PIPELINE_BOTTLENECK", "metrics": {"current_probe_verified": True, "coverage_degraded": False}}
        for i in range(4)
    ]
    monkeypatch.setattr(m, "_load_json", lambda _p: _insight(rows))
    previous = {"tokens": [{"token_address": "T0", "selection_lane": "WALLET_COVERAGE_GAP_PRIORITY"}, {"token_address": "T1", "selection_lane": "WALLET_COVERAGE_GAP_PRIORITY"}]}
