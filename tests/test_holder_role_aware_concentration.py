...
def test_exact_pair_is_excluded_and_replaced_by_next_holder():
    ...
    # Real top10 is ranks 2-11, not gross top10 minus pair without replacement.
...
def test_high_confidence_cex_custody_is_not_a_single_whale_but_is_tracked():
    ...
    holders = [
        {"owner": custody, "pct": 42.59},
        {"owner": pair, "pct": 21.66},
    ] + _holders(3.08, 1.21, 1.01, 1.0, 0.95, 0.9, 0.85, 0.8, 0.75, 0.7)
    registry = [{
        "chain": "BSC",
        "address": custody,
        "role": "CEX_CUSTODY",
...
def test_low_confidence_cex_label_remains_counted_fail_closed():
    ...
    first = d["holders"][0]
    assert first["holder_role"] == "CEX_CUSTODY"
    assert first["excluded_from_whale_concentration"] is False
...
def test_cex_custody_monitor_does_not_claim_funds_are_locked(monkeypatch):
    ...
    pair = "0x9b487fe6c7f4d62df0a63dbfb0b56b60e55c55f5"
    holders = [
        {"owner": custody, "pct": 42.0},
        {"owner": pair, "pct": 20.0},
    ] + _holders(5, 4, 3, 2, 1, 1, 1, 1, 1, 1, 1, 1)
    ...
    assert out["holder_role_policy"].startswith("ONLY_EXACT_HIGH_CONFIDENCE")
