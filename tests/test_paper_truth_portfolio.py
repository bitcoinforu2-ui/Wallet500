...
def test_no_entry_without_firm_quote():
    ...
    row = candidate()
    ledger, summary = reconcile(ledger, [row], {}, {}, '2026-08-29T00:01:00+00:00')
    assert summary['open_positions'] == 0
    assert summary['cash_usd'] == 100.0
    assert summary['entry_quote_verified_positions'] == 0


def test_entry_books_only_verified_router_quantity():
    ledger = initial_ledger('2026-08-29T00:00:00+00:00')
    ...
    assert p['entry_quote_proof_level'] == 'FIRM_ENTRY_TEST'


def test_exit_signal_does_not_fabricate_sale_without_quote():
    ledger = initial_ledger('2026-08-29T00:00:00+00:00')
    row = candidate()
    key = 'bsc|0xabc|0xpair'
    ledger, _ = reconcile(ledger, [row], {key: entry_quote(1.0)}, {}, '2026-08-29T00:01:00+00:00')
    ledger, summary = reconcile(ledger, [], {}, {}, '2026-08-29T00:02:00+00:00')
    assert summary['open_positions'] == 1
...
def test_verified_exit_books_realized_quote_profit():
    ledger = initial_ledger('2026-08-29T00:00:00+00:00')
    row = candidate()
    key = 'bsc|0xabc|0xpair'
    ledger, _ = reconcile(ledger, [row], {key: entry_quote(1.0)}, {}, '2026-08-29T00:01:00+00:00')
    ...
    assert summary['open_positions'] == 0
    assert summary['closed_quote_verified_positions'] == 1
