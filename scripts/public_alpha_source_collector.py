...
UA='Wallet500-PublicAlphaCollector/1.1'
SOL_LINK=re.compile(r'(?:/terminal/solana/|/t/)([1-9A-HJ-NP-Za-km-z]{32,44})')
EVM_LINK=re.compile(r'(?:/terminal/(?:ethereum|eth|base|arbitrum|bsc|optimism|polygon)/|/t/)(0x[a-fA-F0-9]{40})')
...
def main():
 ...
 existing={(str(x.get('source')),str(x.get('network')),str(x.get('contract')).lower()) for x in calls};added=0;health={};seen=state.setdefault('seen_contracts',{})
 for s in cfg.get('sources') or []:
  if not s.get('enabled'):continue
  body=text(s['url']);sid=s['id'];observed=now()
  if body is None:health[sid]={'status':'ERROR','observed_at':observed};continue
  candidates=[('eth',x) for x in set(EVM_LINK.findall(body))]+[('solana',x) for x in set(SOL_LINK.findall(body)) if not x.startswith('0x')]
  accepted=0
  for network,ca in candidates[:250]:
   ...
   if ek not in existing:calls.append(row);existing.add(ek);added+=1;accepted+=1
